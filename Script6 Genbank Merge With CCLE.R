#################################################################################################################
# Script 6: Identifies mutations within CCLE database that occur in hotspot kinase motifs specified in Script 3.#
#                                                                                                               #
# Automatic search function to detect mutations in the CCLE database that occur within hotspot kinase residues  #
...
########################
# Merge Genbank Data with the CCLE Database and formatting
########################

# Merge with combined CCLE data.
comboDATA <- read.csv (file = 'SOURCE_ccletrimmed_namechanged_HRT18removed_HEC1Bremoved.csv')
colnames(comboDATA)[8] <- "case_id"  #can i manually do this?
kinaseLoc <- genbankkinases


# Remove all non missense mutations.
...
# Extract mutation position data from comboDATA.
...
########################
# Calculates the number of mutations occuring within hotspot residues from the CCLE database.
########################
...
merged_G1 <- merged
write.csv (merged_G1, file='outputs/ccle/merged_GxGxxG_G1_all.csv')
...
merged <- subset(merged, DUP == 'TRUE')
merged_G2 <- merged
write.csv (merged_G2, file='outputs/ccle/merged_GxGxxG_G2_all.csv')
...
merged <- subset(merged, DUP == 'TRUE')
merged_G3 <- merged
write.csv (merged_G3, file='outputs/ccle/merged_GxGxxG_G3_all.csv')
...
merged <- subset(merged, DUP == 'TRUE')
merged_K <- merged
write.csv (merged_K, file='outputs/ccle/critical_lysine_all.csv')
...
merged <- subset(merged, DUP == 'TRUE')
merged_DFG_D <- merged
write.csv (merged_DFG_D, file='outputs/ccle/merged_DFG_D_all.csv')
...
merged <- subset(merged, DUP == 'TRUE')
merged_DFG_F <- merged
write.csv (merged_DFG_F, file='outputs/ccle/merged_DFG_F_all.csv')
...
merged <- subset(merged, DUP == 'TRUE')
merged_DFG_G <- merged
write.csv (merged_DFG_G, file='outputs/ccle/merged_DFG_G_all.csv')
...
merged <- subset(merged, DUP == 'TRUE')
merged_HRD_H <- merged
write.csv (merged_HRD_H, file='outputs/ccle/merged_HRD_H_all.csv')
...
merged <- subset(merged, DUP == 'TRUE')
merged_HRD_R <- merged
write.csv (merged_HRD_R, file='outputs/ccle/merged_HRD_R_all.csv')
...
merged <- subset(merged, DUP == 'TRUE')
merged_HRD_D <- merged
write.csv (merged_HRD_D, file='outputs/ccle/merged_HRD_D_all.csv')
...
write.csv (merged_HRD_Dp5, file='outputs/ccle/merged_HRD_Dp5_all.csv')
...
merged <- subset(merged, DUP == 'TRUE')
merged_HRDm6 <- merged
write.csv (merged_HRDm6, file='outputs/ccle/merged_HRDm6_all.csv')
...
merged <- subset(merged, DUP == 'TRUE')
merged_APE_A <- merged
write.csv (merged_APE_A, file='outputs/ccle/merged_APE_A_all.csv')
...
merged <- subset(merged, DUP == 'TRUE')
merged_APE_P <- merged
write.csv (merged_APE_P, file='outputs/ccle/merged_APE_P_all.csv')
...
merged <- subset(merged, DUP == 'TRUE')
merged_APE_E <- merged
write.csv (merged_APE_E, file='outputs/ccle/merged_APE_E_all.csv')
...
write.csv (merged_correctedG, file='outputs/ccle/merged_correctedG_all.csv')
...
write.csv (merged_saltbridge, file='outputs/ccle/merged_saltbridge_all.csv')
...
write.csv (merged_Kminus2, file='outputs/ccle/critical_lysine_minus2.csv')
...
write.csv (total, file ='outputs/ccle/total_ALLTRANSCRIPTS.csv')

freqscreen <- table(total$Gene)
write.csv (freqscreen, file ='SOURCE_cclefreqscreen_ALLTRANSCRIPTS.csv')         
